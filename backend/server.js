...
app.put("/profile", verifyToken, async (req, res) => {
  try {
    ...
    if (!name || !gender || !category) {
      return res
        .status(400)
        .json({ message: "Name, gender, and category are required" });
    }
...
app.post("/upgrade-request", verifyToken, async (req, res) => {
  try {
    ...
    if (userData.role !== "end-user") {
      return res
        .status(400)
        .json({ message: "Only end-users can request an upgrade" });
    }
    ...
    res
      .status(201)
      .json({ message: "Upgrade request created", request: data[0] });
  } catch (err) {
...
app.get("/tickets", verifyToken, async (req, res) => {
  try {
    const { data, error } = await supabase
      .from("tickets")
      .select(
        "id, question, description, tags, username, status, upvote_count, conversation_count, created_at"
      )
...
app.get("/agent/tickets", verifyToken, verifyAgent, async (req, res) => {
  try {
    const { data, error } = await supabase
      .from("tickets")
      .select(
        "id, question, description, tags, username, status, upvote_count, conversation_count, created_at, assigned_agent"
      )
      .order("created_at", { ascending: false });
  ...
  async (req, res) => {
    try {
      ...
      if (error) {
        return res
          .status(400)
          .json({ message: "Error updating ticket status: " + error.message });
      }
  ...
  async (req, res) => {
    try {
      ...
      if (error) {
        return res
          .status(400)
          .json({ message: "Error adding comment: " + error.message });
      }
...
app.post("/tickets/:id/upvote", verifyToken, async (req, res) => {
  try {
    ...
    if (existingUpvote) {
      return res
        .status(400)
        .json({ message: "You have already upvoted this ticket" });
    }
    ...
    if (error) {
      return res
        .status(400)
        .json({ message: "Error adding upvote: " + error.message });
    }
    ...
    if (updateError) {
      return res
        .status(400)
        .json({
          message: "Error updating upvote count: " + updateError.message,
        });
    }
  ...
  try {
    ...
    if (!userData || (userData.role !== "admin" && userData.role !== "agent")) {
      return res
        .status(403)
        .json({
          message: "Forbidden: Only admins and agents can create categories",
        });
    }
...
app.get("/end-user/my-tickets", verifyToken, async (req, res) => {
  try {
    ...
    const { data, error } = await supabase
      .from("tickets")
      .select(
        "id, question, description, tags, status, upvote_count, conversation_count, created_at, assigned_agent"
      )
...
// GET /admin/upgrade-requests - Get all pending upgrade requests
app.get("/admin/upgrade-requests", verifyToken, async (req, res) => {
  try {
    ...
    // Fetch pending upgrade requests with user details
    const { data, error } = await supabase
      .from("approval")
      .select(
        `
...
// PUT /admin/upgrade-requests/:id - Approve or reject upgrade request
app.put("/admin/upgrade-requests/:id", verifyToken, async (req, res) => {
  try {
    ...
    if (updateErr) {
      return res
        .status(400)
        .json({ message: "Error updating request: " + updateErr.message });
    }
    ...
    if (status === "approved") {
      ...
      if (roleErr) {
        ...
        return res
          .status(400)
          .json({ message: "Request updated but failed to update user role" });
      }
    }
...
app.get("/admin/users", verifyToken, async (req, res) => {
  try {
    ...
    const { data, error } = await supabase
      .from("users")
      .select("id, name, email, role, category, created_at")
      .neq("id", req.user.id)
      .order("created_at", { ascending: false });
...
app.put("/admin/users/:id/role", verifyToken, async (req, res) => {
  try {
    ...
    // Validate role
    const validRoles = ["end-user", "agent"];
    ...
    if (error) {
      return res
        .status(400)
        .json({ message: "Error updating user role: " + error.message });
    }

    res
      .status(200)
      .json({ message: "User role updated successfully", user: data });
  } catch (err) {
